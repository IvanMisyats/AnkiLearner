#!/usr/bin/env bash
# AnkiLearner deploy: pull the current image and reconcile the running stack.
#
# Installed on the VPS as /usr/local/bin/anki-deploy, owned root:root 0755 — the `anki` user runs
# it but cannot edit it, so a stolen CI deploy key cannot turn "redeploy" into "run anything".
#
# Two callers, same script:
#   1. CI, instantly, through a forced-command SSH key:
#        ~anki/.ssh/authorized_keys:
#        restrict,command="/usr/local/bin/anki-deploy" ssh-ed25519 AAAA... anki-ci-deploy
#      $SSH_ORIGINAL_COMMAND is ignored on purpose — whatever the client asks for, this runs.
#   2. anki-deploy.timer (user unit), every 5 minutes, as a fallback.
#
...
# Non-login shell (SSH forced command, systemd user unit): the rootless Docker environment is
# not inherited from a profile, so set it explicitly.
export XDG_RUNTIME_DIR="/run/user/$(id -u)"
export DOCKER_HOST="unix://${XDG_RUNTIME_DIR}/docker.sock"

log()  { printf '[anki-deploy %s] %s\n' "$(date -u +'%H:%M:%SZ')" "$*"; }
fail() { log "ERROR: $*"; exit 1; }

# Serialise: the timer and a CI trigger can land at the same moment.
exec 9>"${LOCK_FILE}"
...
IMAGE="$(compose config --images 2>/dev/null | grep 'ankilearner' | head -1)"
[[ -n "${IMAGE}" ]] || fail "could not resolve the api image from docker-compose.yml"
...
running="$(docker ps --quiet --filter 'name=ankilearner-api')"
...
# `up -d` is declarative: it recreates only what changed. No `down` first — that would turn every
# deploy into avoidable downtime. EF migrations run in the API's startup path.
